...
<div id="wrap">
    ...
    <div class="primary">
        <h1>代理列表</h1>
        <form id="searchForm" action="${ctx}/Agency/listOne"  method="get">
            <input type="hidden" name="pageIndex" value="${page.pageIndex}"/>
            <input type="hidden" name="pageSize" value="${page.pageSize}"/>

            <div class="form_Total">
                <ul class="form_ul">
                    <li style="width: 11.1%">序列</li>
                    <li style="width: 11.1%">代理名</li>
                    <li style="width: 11.2%">余额</li>
                    <li style="width: 11.1%">上级代理</li>
                    <li style="width: 11.1%">上级代理费率</li>
                    <li style="width: 11.1%">上上级代理</li>
                    <li style="width: 11.1%">上上级代理费率</li>
                    <li style="width: 11.1%">上上上级代理</li>
                    <li style="width: 11.1%">上上上级代理费率</li>
                    <%--<li>状态</li>
                    <li>操作</li>--%>
                </ul>
                <c:forEach items="${page.list}" var="agency" varStatus="status">
                    <ul class="col-ul">
                        <li style="width: 11.1%">${status.index + 1}</li>
                        <li style="width: 11.1%">${agency.company}</li>
                        <c:forEach items="#{qrchannlList}" var="QR">
                            <c:if test="${QR.merchantId eq agency.id}">
                                <li style="width: 11.2%">${QR.rechargeAmount}</li>
                            </c:if>
                        </c:forEach>
                        <c:forEach items="#{AgencyList}" var="AL">
                            <c:if test="${AL.id eq agency.superiorId}">
                                <li style="width: 11.1%">${AL.company}</li>
                            </c:if>
                        </c:forEach>
                        <li style="width: 11.1%">${agency.superiorContribution}</li>
                        <c:choose>
                            <c:when test="${0 eq agency.greatGrandfatherId}">
                                <li style="width: 11.1%">无</li>
                            </c:when>
                            <c:when test="${0 != agency.greatGrandfatherId}">
                                <c:forEach items="#{AgencyList}" var="AL">
                                    <c:if test="${AL.id eq agency.grandfatherId}">
                                        <li style="width: 11.1%">${AL.company}</li>
                                    </c:if>
                                </c:forEach>
                            </c:when>
                        </c:choose>
                        <li style="width: 11.1%">${agency.grandfatherContribution}</li>
                        <c:choose>
                            <c:when test="${0 eq agency.greatGrandfatherId}">
                                <li style="width: 11.1%">无</li>
                            </c:when>
                            <c:when test="${0 != agency.greatGrandfatherId}">
                                <c:forEach items="#{AgencyList}" var="AL">
                                    <c:if test="${AL.id eq agency.greatGrandfatherId}">
                                        <li style="width: 11.1%">${AL.company}</li>
                                    </c:if>
                                </c:forEach>
                            </c:when>
                        </c:choose>
                        <li style="width: 11.1%">${agency.greatGrandfatherContribution}</li>
                    </ul>
                </c:forEach>
            </div>
        </form>
