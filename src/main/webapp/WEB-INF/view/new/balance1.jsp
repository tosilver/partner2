                ...
                <form id="searchForm" class="form-horizontal" role="form" action="${ctx}/malladdress/list" method="get">
                    ...
                    <div class="tile-body no-vpadding">
                        ...
                        <table class="table table-custom table-sortable">
                            ...
                            <tbody>
                            <c:forEach items="${page.list}" var="mallAdress" varStatus="status">
                                <tr>
                                    <td>${status.index + 1}</td>
                                    <td>${mallAdress.mallName}</td>
                                    <td>${mallAdress.rechargeAmount}</td>
                                    <td>${mallAdress.frozenCapitalPool}</td>
                                    <td>${mallAdress.status== 0 ? "<font color='#FF0000'>停用</font>" : "可用"}</td>
                                </tr>
                            </c:forEach>
                            </tbody>
                        </table>
